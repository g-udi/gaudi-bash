...
bash-it is installed by running one of the following commands in your terminal. You can install this via the command-line with either `curl` or `wget`, whichever is installed on your machine.

```shell
sh -c "$(curl -fsSL https://raw.githubusercontent.com/ahmadassaf/bash-it/master/install.sh)"
```
...
```shell
curl -Lo install.sh https://raw.githubusercontent.com/ahmadassaf/bash-it/master/install.sh
sh install.sh
```
